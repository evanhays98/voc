...
const getIntensityClass = (total: number) => {
  ...
  if (total <= 5) return "bg-indigo-200";
  if (total <= 10) return "bg-indigo-400";
  return "bg-indigo-600";
};

export const StreakCalendar = ({ dailyActivity }: StreakCalendarProps) => {
      ...
      <div className="grid grid-cols-7 gap-1">
        {days.map(({ key, total }) => (
          <div
            ...
            className={`aspect-square rounded-sm transition-colors ${getIntensityClass(total)}`}
          />
        ))}
      </div>
